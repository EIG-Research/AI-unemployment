...
org_charts = cps_org %>%
  ...
  summarise(
    ...
    chgocc_lowerexp_AIOE_quint_wgt = weighted.mean(chgocc_lowerexp_AIOE_quint_wgt, finalwgt, na.rm = TRUE),
    .groups = "drop"
  )
...
# read in EIG logo for font formatting
...
crosswalk_version =  "(Felten et al. (2021))."
...
p_unemp <- ggplot(p_unemp_df, aes(x = qdate_date, y = unemp, color = AIOE_quint_wgt, linetype = AIOE_quint_wgt)) +
  geom_smooth(
  ...
  labs(
    y = "unemployment (%)",
    x = "",
    title = "Figure 1. Unemployment rate by AI exposure quintile",
    caption = paste("Note: AI exposure estimate crosswalked using weighted abilities-based exposure ",crosswalk_version,"\nSource: AI exposure from", var_source, "Quarterly unemployment from CPS.")
  ) +
...
# Add logo to plot
p_unemp <- ggdraw(p_unemp) +  draw_grob(logo_grob)
p_unemp
setwd(output_path)
...
lfpr_moving_plot <- ggplot(org_charts, aes(x = as.Date(yq), y = chglf, color = factor(AIOE_quint_wgt))) +
  ...
  labs(
    ...
    x = NULL,
    title = "Figure 2: Share of workers exiting the labor force by AI exposure quintile",
    caption = paste("Note: Graphed are 1-year changes, where the most recent data represents change from Q4 2023 to Q4 2024.\n  AI exposure estimate crosswalked",crosswalk_version,"\nSource: AI exposure from", var_source, "Quarterly labor force participation data from CPS Outgoing \n  Rotational Group sample.")
  ) +
  theme_minimal(base_size = 12) +
  theme(
    plot.title = element_text(family = "sans", face = "bold", color = "#387e65", size = 14, hjust = -1.1),
    plot.background = element_rect(fill = "white", color = NA),
    panel.background = element_rect(fill = "white", color = NA),
    legend.position = "bottom",
    legend.title = element_text(size = 10),
...
lfpr_moving_plot <- ggdraw(lfpr_moving_plot) +  draw_grob(logo_grob)
...
occ_switching_plot <- ggplot(org_charts, aes(x = as.Date(yq), y = chgocc, color = factor(AIOE_quint_wgt))) +
   ...
   labs(
     ...
     x = NULL,
     title = "Figure 3: Share of workers switching occupation by AI exposure quintile",
     caption = paste("Note: Graphed are 1-year changes, where the most recent data represents change from Q4 2023 to Q4 2024.\n  AI exposure estimate crosswalked",crosswalk_version,"\nSource: AI exposure from", var_source, "Quarterly occupation data from CPS Outgoing Rotational \n  Group sample.")
   ) +
   theme_minimal(base_size = 12) +
   theme(
     plot.title = element_text(family = "sans", face = "bold", color = "#387e65", size = 14, hjust = -1.01),
     plot.background = element_rect(fill = "white", color = NA),
     panel.background = element_rect(fill = "white", color = NA),
     legend.position = "bottom",
     legend.title = element_text(size = 10),
...
occ_switching_plot <- ggdraw(occ_switching_plot) +  draw_grob(logo_grob)
...
plot_switching_lower_exposure <- org_charts %>%
  ...
  labs(x = "",
       ...
       title = "Figure 4: Probability the most AI-exposed workers switch to lower-exposure occupations",
  ...
  theme(
    plot.title = element_text(family = "sans", face = "bold", color = "#387e65", size = 14, hjust = 1.4),
...
plot_switching_lower_exposure <- ggdraw(plot_switching_lower_exposure) +  draw_grob(logo_grob)


setwd(output_path)
ggsave("7_changing_lower_exposure.png",  width = 8.5, height = 5, dpi = 300)
...
qcew_df_cumulative <- qcew_df %>%
  ...
  mutate(yq_date = as.Date(as.yearqtr(yq, format = "%Y Q%q")),
         year_label = year(yq_date))


plot_employment <- ggplot(
  qcew_df_cumulative %>% arrange(aioe_q5, yq),
  aes(x = yq_date, y = qtr_emplvl, color = factor(aioe_q5))
) +
  ...
  labs(
    ...
    x = "",
    title = "Figure 5: Industry employment by AI exposure quintile",
  ...
  scale_x_date(
    ...
    labels = function(x) format(x, "%Y")  # Only show the year
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(family = "sans", face = "bold", color = "#387e65", size = 14, hjust = -0.18),
    legend.position = "bottom",
    plot.caption.position = "plot",
    plot.caption = element_text(hjust = 0)
  ) +
...
plot_employment <- ggdraw(plot_employment) +  draw_grob(logo_grob)


setwd(output_path)
ggsave("8_employment_by_quintile.png",plot = plot_employment, width = 8, height = 5, dpi = 300, bg = "white")
...
plot_recent_grads <-
  ...
  labs(
    title = "Figure 6: Recent graduates unemployment rate by AI exposure",
    subtitle = "By exposure category",
    x = NULL,
    y = "unemployment (%)",
    color = "AI exposure and \ndegree group",
    linetype = "AI exposure",
    caption = "Note: Sample covers 22–27 year olds. AI exposure estimate crosswalked using weighted abilities-based \n  exposure (Felten et al., 2021).\nSource: AI exposure from Felten et al. (2021), unemployment rates from CPS."
  ) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  theme_minimal(base_size = 12) +
  theme(
    plot.title = element_text(family = "sans", face = "bold", color = "#387e65", size = 14, hjust = -0.4),
...
plot_recent_grads <- ggdraw(plot_recent_grads) +  draw_grob(logo_grob)
...
plot_info_sector_jobs <-
 ...
 theme_minimal() +
  ...
  labs(title = "Figure 7: Information sub-sector employment",
       y = "jobs (thousands)",
       caption = "Note: AI uptake as of June 2025 in the legend. Estimates not available for the telecomunications sector.\nSource: BLS Current Employment Statistics, BTOS for AI uptake statistics.",
       x = "") +
  scale_color_manual(
    values = c(
  ...
  theme(
    plot.title = element_text(family = "sans", face = "bold", color = "#387e65", size = 14, hjust = -0.19),
    legend.position = "bottom",
...
plot_info_sector_jobs <- ggdraw(plot_info_sector_jobs) +  draw_grob(logo_grob)


setwd(output_path)
ggsave("10_employment_3digit_nacis.png", plot = plot_info_sector_jobs, width = 8, height = 5, dpi = 300, bg = "white")
...
change_employment_main <- ggplot(results, aes(x = label, y = e)) +
  geom_bar(stat = "identity", width = 0.75, fill = "#b3d6dd") +
  geom_errorbar(aes(ymin = dwn, ymax = up), width = 0.2) +
  labs(
    x = NULL,
    y = "Change in Unemployment",
    title = "Figure 8: Change in unemployment, most AI exposed workers relative to all other workers",
    caption = "Note: Change in unemployment for periods 2024/2025 relative to 2022/2023. \nSource: CPS, exposure from Felten et al. (2021), Eloundou et al. (2024), Eisfeldt et al. (2024), Webb (2022)."
  ) +
  scale_y_continuous(limits = c(-0.2, 0.5), breaks = seq(-0.2, 0.5, by = 0.2)) +
  theme_minimal(base_size = 14) +
  theme(
    plot.title = element_text(family = "sans", face = "bold", color = "#387e65", size = 14, hjust = 1.05),
...
change_employment_main  <- ggdraw(change_employment_main) +  draw_grob(logo_grob)

setwd(output_path)
ggsave("11_d_in_d.png",  width = 8.5, height = 5, dpi = 300, bg = "white")

# agglomerations short piece version
change_employment_agglom <- ggplot(results, aes(x = label, y = e)) +
  geom_bar(stat = "identity", width = 0.75, fill = "#b3d6dd") +
  geom_errorbar(aes(ymin = dwn, ymax = up), width = 0.2) +
  labs(
    x = NULL,
    y = "Change in Unemployment",
    title = "Figure 2: Change in unemployment, most AI exposed workers relative to all other workers",
    caption = "Note: Change in unemployment for periods 2024/2025 relative to 2022/2023. \nSource: CPS, exposure from Felten et al. (2021), Eloundou et al. (2024), Eisfeldt et al. (2024), Webb (2022)."
  ) +
  scale_y_continuous(limits = c(-0.2, 0.5), breaks = seq(-0.2, 0.5, by = 0.2)) +
  theme_minimal(base_size = 14) +
...
change_employment_agglom  <- ggdraw(change_employment_agglom) +  draw_grob(logo_grob)


setwd(agglom_path)
ggsave("2_change_employment_reg.png", width = 8.5, height = 5, dpi = 300, bg = "white")
