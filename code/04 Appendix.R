...
ggplot(all_data_with_overall, aes(x = qdate_date, y = unemp, color = source, linetype = source)) +
  ...
  labs(
    title = "Unemployment rate for the most exposed workers across measures",
    y = "unemployment (%)",
    x = "",
    caption = "Note: Each line shows quintile 5 workers under a different AI exposure model. Grey dotted line shows unemployment for all workers.\nSource: CPS, exposure from Felten et al. (2021), Eloundou et al. (2024), Eisfeldt et al. (2024), Webb (2022)."
  ) +
...
setwd(output_path)
ggsave("1_unemp_all_measures.png", width = 8, height = 5, dpi = 300)
...
linetypes <- c(
  ...
  "Eisfeldt" = "solid",
  "Webb" = "solid",
  "All other workers" = "dotted"
)

# Plot
ggplot(all_lfpr_df, aes(x = as.Date(yq), y = toue, color = source, linetype = source)) +
  ...
  labs(
    title = "Share of workers exiting the labor force by AI exposure (quintile 5 only)",
    y = "% exiting labor force",
    x = NULL,
    caption = "Note: Quarter 2 2020 values above 2.5% removed. Grey dotted line for quintiles 1–4 overall.\nSource: CPS ORG from Economic Policy Institute, exposure from Felten et al. (2021), Eloundou et al. (2024), \nEisfeldt et al. (2024), Webb (2022)."
  ) +
  theme_minimal(base_size = 12) +
  theme(
    plot.background = element_rect(fill = "white", color = NA),
...
setwd(output_path)
ggsave("2_lfpr_switching_quint5.png", , width = 8, height = 5, dpi = 300)
...
ggplot(all_occ_df, aes(x = as.Date(yq), y = chgocc, color = source, linetype = source)) +
  geom_smooth(method = "loess", se = FALSE, span = 0.5, linewidth = 0.6) +
  ...
  scale_y_continuous(labels = scales::percent_format()) +
  labs(
    title = "Share of workers changing occupation by AI exposure (quintile 5 only)",
    y = "% changing occupation",
    x = NULL,
    caption = "Note: Grey dotted line for quintiles 1–4 overall.\nSource: CPS ORG from Economic Policy Institute, exposure from Felten et al. (2021), Eloundou et al. (2024), \nEisfeldt et al. (2024), Webb (2022)."
  ) +
  theme_minimal(base_size = 12) +
  theme(
    plot.background = element_rect(fill = "white", color = NA),
...
setwd(output_path)
ggsave("3_occ_switching_quint5.png", , width = 8, height = 5, dpi = 300)
...
ggplot(combined_data, aes(x = as.Date(yq), y = value, color = exposure_label, linetype = exposure_label)) +
...
ggplot(org_charts_long_multi, aes(x = as.Date(yq), y = value, color = exposure_label)) +
  geom_smooth(method = "loess", se = FALSE, span = 0.5, linewidth = 0.5) +
  geom_point(alpha = 0.3, size = 1.5) +
  theme_minimal(base_size = 12) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  ...
  labs(
    ...
    subtitle = "Among most-exposed workers (quintile 5), by AI exposure measure",
    color = "AI Exposure Source",
    caption = "Note: Each line shows quintile 5 workers under a different AI exposure model.\nSource: CPS ORG from the Economic Policy Institute, exposure from Felten et al. (2021), Eloundou et al. (2024), \nEisfeldt et al. (2024)."
  ) +
  theme(
    legend.position = "bottom",
    legend.title = element_text(size = 10),
...
setwd(output_path)
ggsave("4_switching_low_xposure_quint5.png",  width = 8, height = 5, dpi = 300)
...
plot_ai_use <- ggplot(btos_plot_df, aes(x = date, y = use_ai)) +
  ...
  labs(
    x = "", 
    y = "share using AI",
    title = "Percent of businesses using AI (in the last 2 weeks)",
...
setwd(output_path)
ggsave("5_ai_use_btos.png", plot = plot_ai_use, width = 8, height = 5, dpi = 300, bg = "white")
...
ggplot(adoption_industry_graph, aes(x = Sector_label, y = response, fill = question)) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7) +
  labs(
    x = "",
    y = "",
    fill = "",
    title = "Share of companies that report using AI in...",
    legend = NULL,
...
setwd(output_path)
ggsave("6_sector_ai_use.png", width = 8, height = 5, dpi = 300, bg = "white")
